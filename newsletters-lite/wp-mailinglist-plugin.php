...
if (!class_exists('wpMailPlugin')) {
	class wpMailPlugin extends wpMailCheckinit {
		...
		function __construct() {			
			...
			if (class_exists('WPML_WP_Import_Process')) {
				$this -> import_process = new WPML_WP_Import_Process();
			}
			
			include($this -> plugin_base() . DS . 'vendors' . DS . 'processing' . DS . 'wp-queue-process.php');
			if (class_exists('WPML_WP_Queue_Process')) {
				$this -> queue_process_1 = new WPML_WP_Queue_Process();
			}
			
			include($this -> plugin_base() . DS . 'vendors' . DS . 'processing' . DS . 'wp-queue-process_2.php');
			if (class_exists('WPML_WP_Queue_Process_2')) {
				$this -> queue_process_2 = new WPML_WP_Queue_Process_2();
			}
			
			include($this -> plugin_base() . DS . 'vendors' . DS . 'processing' . DS . 'wp-queue-process_3.php');
			if (class_exists('WPML_WP_Queue_Process_3')) {
				$this -> queue_process_3 = new WPML_WP_Queue_Process_3();
			}
			
			include($this -> plugin_base() . DS . 'vendors' . DS . 'processing' . DS . 'wp-dbupdate-process.php');
			if (class_exists('WPML_WP_Dbupdate_Process')) {
				$this -> dbupdate_process = new WPML_WP_Dbupdate_Process();
			}
		}
		...
		function qp_do_crons() {
			do_action('WPML_WP_Queue_Process_cron');
			do_action('WPML_WP_Queue_Process_2_cron');
			do_action('WPML_WP_Queue_Process_3_cron');
		}
...
function qp_scheduling() {
		...
		function after_plugin_row($plugin_name = null) {
            ...
            if (is_array($validation_status)  && $validation_status['expired'] && apply_filters('newsletters_updates', true)) { ?>
                <tr class="plugin-update-tr active" id="<?php echo $this -> plugin_name; ?>-update" data-slug="<?php echo $this -> plugin_name; ?>" data-plugin="<?php echo $this -> plugin_file; ?>">
					<td colspan="5" class="plugin-update colspanchange">
						<div class="update-message notice inline notice-warning notice-alt">
							<p>
								<?php echo sprintf(__('Your serial key has expired. You are now running %s LITE. To remove limits without renewing, deactivate and remove this free version, then go to %s to download, install, and activate the paid version, or contact us to get that version, if unavailable. Remaining with %s LITE, along with an expired license, prevents PRO features from being accessible.', 'wp-mailinglist') , $this->name, '<a target="_blank" href="https://tribulant.com/downloads/">' . __('https://tribulant.com/downloads/', 'wp-mailinglist') . '</a>' , $this->name); ?>
								<br/>
								<?php echo __('Explanation: Free version users would previously have access to lifetime updates with an expired license. Due to that, we had to change the way it works. To continue using paid features without renewing, deactivate and remove this free version. You will not lose your data. Then, download, install, and activate our paid version from your account with us on our Downloads page. If you are unable to download it, contact us to get that version.', 'wp-mailinglist'); ?>
							</p>
						</div>
					</td>
				        ...
				        <td colspan="4" class="plugin-update colspanchange">
					        <div class="update-message notice inline notice-warning notice-alt">
                                ...
                                if (!is_array($validation_status) && !$validation_status) {
                                    ...
                                    // echo  sprintf(__('Your serial key has expired. You can still update but you are running %s LITE. To remove limits, you can %s or %s.', 'wp-mailinglist'), $this->name,    '<a target="_blank" href="https://tribulant.com/downloads/">' . __('renew your license', 'wp-mailinglist') . '</a>', '<a target="_blank" href="https://tribulant.com/downloads/">' . __('upgrade to Lifetime', 'wp-mailinglist') . '</a>');
								}
        ...
        function ajax_dismiss_forever_notice() {
            ...
            if (current_user_can('newsletters_welcome')) {
                ...
                $this -> update_option('dismiss_forever-showmessage_ratereview', true);
            }
		...
		function ajax_subscribe() {
			...
			//Apply a filter so users can code whether the subscribers should be added or not after a honeypot chekc
			$honeypot_filter = apply_filters( 'wpml_subscriber_add_new', true );
			if (!$honeypot_filter )
			{
				exit ();
				die();
			...
			if ($subscriber_id = $Subscriber -> optin($_POST)) {
				...
				if (empty($_POST['form_id'])) {
                    ...
                    if (!empty($instance['acknowledgement']) && WPMAIL() -> language_do()) {
                        $language_live =   WPMAIL() -> language_current();
                        $instance['acknowledgement'] = WPMAIL() -> language_use($language_live, $instance['acknowledgement']);
                    }
		...
		function ajax_dkimwizard() {
			...
			switch ($_POST['goto']) {
				case 'step2'				:
					$this -> render('dkim' . DS . 'step2', array('domain' => wp_kses_post($_POST['domain']), 'selector' => wp_kses_post($_POST['selector']), 'public' => wp_kses_post($_POST['public']), 'private' => wp_kses_post($_POST['private'])), true, 'admin');
					break;
				case 'step3'				:
					$this -> render('dkim' . DS . 'step3', array('domain' => wp_kses_post($_POST['domain']), 'selector' => wp_kses_post($_POST['selector']), 'public' => wp_kses_post($_POST['public']), 'private' => wp_kses_post($_POST['private'])), true, 'admin');
					break;
				case 'step1'				:
				default 					:
					...
					$rsa = new Crypt_RSA();
					$keys = $rsa -> createKey();
					$this -> render('dkim' . DS . 'step1', array('domain' => wp_kses_post($_POST['domain']), 'selector' => wp_kses_post($_POST['selector']), 'public' => wp_kses_post($keys['publickey']), 'private' => wp_kses_post($keys['privatekey'])), true, 'admin');
					break;
			}
		...
		function ajax_newsletters_autosave_blockeditor() {
			...
			if (!empty($history_id)) {
                ...
                $email_body = $this -> execute_mail($subscriber, false, $subject, $message, $this -> History() -> attachments, $history_id, $eunique, false, "spam_check");

                $output = "";
                $eol = "\n";
                $output .= "Content-Type: ".$email_body-> ContentType."; charset=utf-8".$eol;
                $output .= "Content-Transfer-Encoding: Quoted-printable".$eol;
                $output .= "Content-Disposition: inline".$eol;
				$output .= "Subject: " . $subject . $eol;
				$output .= "Date: " . date("D, d M Y H:i:s O (T)") . $eol;
				$output .= "From: " . $from  . $eol;
				$output .= "To: " . $from  . $eol;
				$output .= "MIME-Version: 1.0 "  . $eol;
	
                $output .= $eol.$email_body->Body.$eol.$eol;

                $spamscore = $this -> spam_score($output, "long");
	...
	<?php // phpcs:ignore ?>
							...
							<?php elseif (empty($spamscore -> success) || $spamscore -> success == false) : ?>
								<p class="newsletters_error"><?php echo json_encode($spamscore->message); ?></p>
        ...
        function ajax_newsletters_autosave($internal_call = false) {
			...
			$eunique = $Html -> eunique($subscriber, $history_id);
			$email_body = $this -> execute_mail($subscriber, false, $subject, $message, $this -> History() -> attachments, $history_id, $eunique, false, "spam_check");
            $spamscore = $this -> spam_score($email_body, "long");
			// Save the 'spamscore'
			if (!empty($spamscore -> score)) {
				$this -> History() -> save_field('spamscore', $spamscore -> score, array('id' => $history_id));
			}
				...
				<div id="spamscore_report">
					<div class="wrap newsletters">
						...
						<?php if (is_wp_error($spamscore)) : ?>
							<p class="newsletters_error"><?php echo esc_html( $spamscore -> get_error_message()); ?></p>
						<?php elseif (empty($spamscore -> success) || $spamscore -> success == false) : ?>
							<p class="newsletters_error"><?php echo (empty($spamscore -> message) ?  esc_html( $spamscore -> message)  : '' ); ?></p>
						<?php else : ?>
	    ...
	    function ajax_spamscore_blockeditor() {
			...
			$message = $this -> render_email('send', array('message' => $content, 'subject' => $subject, 'subscriber' => $subscriber, 'history_id' => $history_id), false, true, true, $theme_id);
			$eunique = $Html -> eunique($subscriber, $history_id);
			$email_body = $this -> execute_mail($subscriber, false, $subject, $message, $history -> attachments, $history_id, $eunique, false);
			$spamscore = $this -> spam_score($subscriber -> email, "long");
	...
	<?php // phpcs:ignore ?>
			...
			if (current_user_can('newsletters_welcome')) {
				if (!empty($_GET['delete'])) {
					...
					if (!empty($host)) {
						...
						delete_transient( $time_key );
						delete_transient( $valid_key );
					...
					if (!empty($_POST)) {
						if (empty($_REQUEST['serialkey'])) { $errors[] = __('Please fill in a serial key.', 'wp-mailinglist'); }
							...
							$host_hash = md5($host);
							if (!empty($host)) {
								delete_transient("wpml" . 'serial_validation_time_' . $host_hash);
								delete_transient("wpml" . 'serial_valid_' . $host_hash);
							}
                ...
                $prefix = "wpml" . 'serial_valid'; // Replace with your desired prefix

                $sql = "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_{$prefix}%'";
                $wpdb->query($sql);

                $sql = "DELETE FROM $wpdb->options WHERE option_name LIKE '_transient_timeout_{$prefix}%'";
                $wpdb->query($sql);
			} else {
				$success = false;
				$errors[] = __('You do not have permission', 'wp-mailinglist');
		...
		function ajax_subscribercountdisplay() {			
			...
			$users_tmp = get_option('newsletters_eligible_users_count_tmp');
			
			if (!empty($subscribercount_query) || !empty($users_tmp)) {

				$total_subscribers = (!empty($subscribercount_query)) ? (count($wpdb -> get_results($subscribercount_query))) : 0;
				$perpage = 50;
				...
				if( !empty($subscribercount_query)) {
					...
					$subscribers_only = $wpdb -> get_results($subscribercount_query);
				...
				if (!empty($users_tmp)) {
					...
					// Assuming $temp_user contains the serialized data from the database
					$temp_user_data = maybe_unserialize($users_tmp);


					?>

					<p><?php echo __('Users', 'wp-mailinglist'); ?></p>

					<table class="widefat">
						<tbody>
							<?php 

								// Check if $temp_user_data is an array and has at least one element
								if (is_array($temp_user_data) && !empty($temp_user_data)) {
								    // Loop through the array (in this case, it should only have one element)
								    foreach ($temp_user_data as $user_item) {
								        // Check if $user_item is an object of the class WP_User
								        if ($user_item instanceof WP_User) {
								            // Access the properties of the WP_User object
								            $user_id = $user_item->ID;
								            $user_email = $user_item->user_email;
											?>
											<tr >
													<td><?php
											            // Output or use the retrieved values
											            echo $user_email;
											        ?>
									       		 </td>
											</tr>
...
<?php // phpcs:ignore ?>
		...
		function ajax_subscribercount($count = true) {
			...
			if (!empty($_POST['roles'])) {
				if ($count_users = count_users()) {		
					...
					foreach($roles as $role){
					    ...
					    $users_list = array_merge($current_role_users, $users_list);
					}
				}
			}
            $query = "";
			...
			if (!empty($users_count)) {
				...
				update_option('newsletters_eligible_users_count_tmp', maybe_serialize($users_list));
			}
...
// phpcs:disable
		...
		function sc_management($atts = array(), $content = null) {
			...
			switch ($method) {
				...
				case 'logout'				:
                    ...
                    wp_add_inline_script( 'jquery', $wpmljavascript );
		...
		function enqueue_scripts() {
			...
			if (is_admin() && !defined('DOING_AJAX')) {
				...
				$execute = apply_filters( 'wpmlloadscripts', true );

				// Check the value of $execute and execute the code only if it's true
				if ( $execute || is_admin()) {
					wp_localize_script('wp-mailinglist', 'newslettersmain', $params);
			...
			/* Front-End Scripts */
				...
				if ( $captcha_type = $this->use_captcha() ) {
					...
					if ( $captcha_type === 'recaptcha' && ! empty( $loadrecaptcha ) ) {

						$functions_variables['captcha'] = 'recaptcha';

						$sitekey   = $this->get_option( 'recaptcha_publickey' );
						$secret    = $this->get_option( 'recaptcha_privatekey' );
						$type_opt  = $this->get_option( 'recaptcha_type' );      // robot | invisible
						$language  = $this->get_option( 'recaptcha_language' ) ?: 'en';
						$theme     = $this->get_option( 'recaptcha_theme' ) ?: 'light';

						/* Multilingual override */
						if ( $this->language_do() && ( $lang = $this->language_current() ) ) {
							$language = $lang;
						}
						...
						$functions_variables += array(
							...
							'recaptcha_type'      => ( $type_opt === 'invisible' ? 'invisible' : 'robot' ),
							'recaptcha_language'  => $language,
							'recaptcha_theme'     => $theme,
						);

						/* ----- Visible (checkbox) -------------------------- */
						if ( $type_opt !== 'invisible' ) {

							wp_enqueue_script(
								'newsletters-recaptcha',
								'https://www.google.com/recaptcha/api.js?hl=' . esc_attr( $language ),
								array(),
								null,
								true
							);

						/* ----- Invisible ― FIXED badge bottom-right -------- */
						} else {

							if ( ! wp_script_is( 'newsletters-recaptcha', 'enqueued' ) ) {

								wp_enqueue_script(
									'newsletters-recaptcha',
									'https://www.google.com/recaptcha/api.js?render=explicit&hl=' .
									esc_attr( $language ) .
									'&onload=newsletterRecaptchaInit',
									array(),
									null,
									true
								);

								/* Render every placeholder once, badge bottom-right */
						...
						$sitekey   = $this->get_option( 'recaptcha3_publickey' );
						$secret    = $this->get_option( 'recaptcha3_privatekey' );
						$language  = $this->get_option( 'recaptcha3_language' ) ?: 'en';
						$score     = $this->get_option( 'recaptcha3_score' )    ?: '0.5';
						...
						$functions_variables += array(
							'recaptcha3_sitekey'   => $sitekey,
							'recaptcha3_secretkey' => $secret,
							'recaptcha3_language'  => $language,
							'recaptcha3_score'     => $score,
						);

						wp_enqueue_script(
							'newsletters-recaptcha3',
							'https://www.google.com/recaptcha/api.js?render=' .
							esc_attr( $sitekey ) .
							'&hl=' . esc_attr( $language ),
							array(),
							null,
							true
						);
				...
				if ($field = $Field -> get($field_id)) {
					...
					if (!empty($form_id) && !empty($form_field)) {
						...
						if (!empty($form_styling['fieldshowlabel'])) {
							...
							if (!empty($field) && !empty($field->type) && $field->type === "special")
							{
								if (!empty($list) && $list === 'checkboxes' )
								{

									if (!empty($includelists)) {
										$lists_temp = $Mailinglist -> select(true, $includelists);
									} else {
										$lists_temp = $Mailinglist -> select(false);
									}

									if(is_array($lists_temp) && count($lists_temp ) < 2)
									{
							...
							//Seperate the Select All from the label. Put it below the List Label
							if( !empty($list) && $list == "checkboxes" && $show_select_all) {
								echo '<label for="' . ((!empty($list) && $list == "checkboxes") ? 'listsselectall' : ( '' . $this -> pre . '-' . $optinid . $field -> slug . '' )) . '" class="control-label' . ((!empty($list) && $list == "checkboxes") ? ' form-inline' : '') . ' ' . $this -> pre . 'customfield ' . $this -> pre . 'customfield' . $field_id . '">' . ((!empty($list) && $list == "checkboxes") ? $listsselectall . ' ' : '') . '&nbsp;' . __('Select All', 'wp-mailinglist')   . '</label> ';
							}
		...
		function inlinestyles($html = null) {
			...
			if (!empty($inlinestyles)) {
				...
				$html = Pelago\Emogrifier\CssInliner::fromHtml($html)->inlineCss()->render();
		...
		function execute_mail($subscriber = null, $user = null, $subject = null, $message = null, $attachments = null, $history_id = null, $eunique = null, $shortlinks = true, $emailtype = "newsletter") {
			...
			if (empty($error)) {
                ...
                if (!apply_filters('newsletters_execute_mail_override', false) || (!empty($emailtype)  && $emailtype === "spam_check")) {
                    if ($mailtype == "smtp" || $mailtype == "gmail" || (!empty($newsletters_presend) && $newsletters_presend == true) || (!empty($emailtype)  && $emailtype === "spam_check")) {
						...
						try {
							...
							if ($this -> get_option('smtpauth') == "Y") {
								...
								$phpmailer -> Password = html_entity_decode($this -> get_option('smtppass'));
							}
                            ...
                            if  (!empty($emailtype)  && $emailtype === "spam_check")
                            {
                                return $phpmailer;
                                die();
                            }
						...
						if( true ) { //) !$found_duplicate ) {
                            //error_log ('test duplicate not found');
		...
		function updating_plugin() {
			...
			if (version_compare($this -> version, $cur_version) === 1) {
				...
				if (version_compare($cur_version, "4.9.7") < 0) {
					global $wpdb, $Subscriber;

					$query = "ALTER TABLE `" . $wpdb -> prefix . $Subscriber -> table  . "`
						ADD COLUMN is_active ENUM('Y','N') NOT NULL DEFAULT 'N' AFTER referer ";
					$wpdb -> query($query);
		...
		function convert_relative_urls_to_absolute($html = null) {
		    $base_url = get_site_url(); // Get the base URL of the website
		    ...
		    if (!empty($html) && !empty($base_url)) {
		        if (class_exists('DOMDocument')) {
		            ...
		            foreach ($dom->getElementsByTagName('a') as $a) {
		                ...
		                if ($href && !filter_var($href, FILTER_VALIDATE_URL) && !preg_match('/^[\#\[]/', $href)) {
				...
				if (!empty($this -> history_id) && !empty($atts['id'])) {
					//if (is_object($this -> Content)) {
						if ($contentarea = $this -> Content() -> find(array('number' => $atts['id'], 'history_id' => $this -> history_id))) {
							/*
							Add a filter like this in your child theme to disable autop in email rendering
							function disable_wpautop_on_mail_rendering($disable) {
								return true; // Return true to disable wpautop
							}
							add_filter('wpml_disable_wpautop_on_mail_rendering', 'disable_wpautop_on_mail_rendering');
							*/
							$disable_wpautop = apply_filters('wpml_disable_wpautop_on_mail_rendering', false);
		                    if ($disable_wpautop) {
		                        // If the filter returns true, skip wpautop
		                        $output = do_shortcode(wp_unslash($contentarea -> content));
		                    } else {
		                        // By default, apply wpautop
