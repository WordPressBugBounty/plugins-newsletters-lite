...
<?php if (empty($success) || $success == false) : ?>
    ...
    <?php }
        ...
        <p><?php echo sprintf(__('Your current serial is valid and working. %s', 'wp-mailinglist') , '<a href="https://tribulant.com/downloads/" target="_blank">' . __('View Downloads', 'wp-mailinglist') . '</a>') ; ?></p>
        ...
        <p style="width:400px;">
            <?php echo sprintf(__('You will continue to receive updates but you will not have access to PRO features. '), ''); ?>
            <?php echo sprintf(__('These limits are in place due to the limitations of free plugins. Using our PRO version bypasses those.%s', 'wp-mailinglist'), ''); ?>
        </p>
        <p style="width:400px">
            <?php echo sprintf(__('To remove limits, install the PRO version from your %s on our website, where you can also renew, upgrade to lifetime, and/or obtain a new serial key. Contact us if your version is not available. Deactivate and delete the LITE version before activating the PRO version. You will not lose any data.' , 'wp-mailinglist') , '<a href="https://tribulant.com/downloads/" target="_blank">' . __('Downloads page', 'wp-mailinglist') . '</a>'); ?>
        </p>
