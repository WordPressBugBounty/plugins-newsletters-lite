...
if (!empty($newsletters_lite_subscriberlimit) && $newsletters_lite_subscriberlimit > 0  && empty($errors)) {
	...
	if ($subscriber_count >= $newsletters_lite_subscriberlimit) {
		if (!is_array($serial_validation_status) && !$serial_validation_status) {
			...
			?>
			<div class="notice notice-error"><p><?php echo sprintf(__('Subscriber limit of %s has been reached. %s to be able to manage unlimited subscribers.', 'wp-mailinglist'), $newsletters_lite_subscriberlimit, '<a href="' . admin_url('admin.php?page=' . $this->sections->lite_upgrade) . '">Upgrade to PRO</a>'); ?></p></div>
			<?php
		...
		if (is_array($serial_validation_status)) {
			...
			?>
			<div class="notice notice-error"><p><?php echo sprintf(__('Your serial key has expired. Subscriber limit of %s has been reached. %s to be able to manage unlimited subscribers.', 'wp-mailinglist'), $newsletters_lite_subscriberlimit, '<a target="_blank" href="https://tribulant.com/downloads/">Renew your license</a>'); ?></p></div>
			<?php
