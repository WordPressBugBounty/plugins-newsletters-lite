...
if (!class_exists('wpMailCheckinit')) {
    class wpMailCheckinit {
        ...
        function ci_initialize() {
            ...
            if ( !is_admin() || (is_admin() && (!is_array($serial_validation_status) || $serial_validation_status))) {
        ...
        function ci_initialization() {
            ...
            $this -> add_filter('block_editor_settings_all', 'block_editor_settings', 10, 2);
        ...
        function ci_serial_valid() {
            ...
            $last_validation_time = get_transient("wpml" . 'serial_validation_time_' . $host_hash);
            $valid_status = get_transient("wpml" . 'serial_valid_' . $host_hash);

            // Check if it's been less than 24 hours since the last validation
            if (!$last_validation_time || (time() - $last_validation_time) >= 86400) {
                // Perform the online serial validation here
                $is_serial_valid = $this->validate_serial_online();
            ...
            if ($valid_status === 'expired' ) {
                $result = array('status' => true, 'expired' => true);
            }
        ...
        function validate_serial_online()
        {
            // Define the serial and product (abbr)
            $serial = $this->ci_get_serial();
            if(empty($serial)) {
                if(isset($_POST['serialkey']) & !empty($_POST['serialkey']))
                {
                    $serial = $_POST['serialkey'];
                }
            }
            $product = 'wpml';

            $blog_url = get_bloginfo('url'); // Get the full blog URL
            $parsed_url = wp_parse_url($blog_url); // Parse the URL into its components
            $host = '';
            ...
            // Build the API URL
            $api_url = "https://tribulant.com/serials/serial_check/{$serial}/{$product}/{$host}";

            // Get the client's IP address
            $clientIP = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '';

            // Define an array of local IP address ranges
            $localIPRanges = array('127.0.0.0/8', '192.168.0.0/16'); // Add more if needed

            // Check if the client's IP is in a local range
            foreach ($localIPRanges as $range) {
                list($subnet, $mask) = explode('/', $range);
                $subnet = ip2long($subnet);
                $mask = ~((1 << (32 - $mask)) - 1);

                if ((ip2long($clientIP) & $mask) == ($subnet & $mask)) {
                    // Client IP is in a local range, return true without making the API request
                    return true;
                }
            }
            ...
            curl_setopt($ch, CURLOPT_TIMEOUT, 20); // Set a 20-second timeout
            ...
            if (curl_errno($ch)) {
                // Handle cURL error, you can log or return an error message here
                return 'invalid';
            }
            ...
            // Check if the API returned 'valid'
            if ($result && $result[0] === 'valid') {
                return 'valid'; // Serial key is valid
            } else if ($result && $result[0] === 'invalid') {
                return 'invalid'; // Serial key is invalid
            } else if ($result && $result[0] === 'expired') {
                return 'expired'; // Serial key is expired
            } else {
                return 'invalid'; // Serial key is invalid in any unexpected restuls
            }
