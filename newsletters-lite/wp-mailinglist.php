...
require_once(NEWSLETTERS_DIR . DS . 'wp-mailinglist-plugin.php');
        ...
        class wpMail extends wpMailPlugin {
            ...
            function phpmailer_init($phpmailer = null) {
                ...
                if (!empty($fromwpml) && $fromwpml == true) {
                    ...
                    if ( WPMAIL()-> language_do()) {
                        ...
                        $smtpfrom = WPMAIL() -> language_use($language_live, $smtpfrom);
                    }
                    ...
                    if ( WPMAIL()-> language_do()) {
                        $language_live =   WPMAIL() -> language_current();
                        $smtpfromname = WPMAIL() -> language_use($language_live, $smtpfromname);
                    }
            ...
            function emailarchive_hook() {
                ...
                if (!empty($emailarchive)) {
                    ...
                    // Remove non-digit characters from the variable
                    $numericValue = preg_replace('/\D/', '', $emailarchive_olderthan);
                    $interval = 90;
                    // Check if the result is not empty (contains digits)
                    if (!empty($numericValue)) {
                        // Convert the numeric value to an integer
                        $interval = intval($numericValue);
                    }
                    ...
                    if (file_exists($outfile_full) && is_writable($outfile_full)) {
                        $query = "SELECT * FROM " . $wpdb->prefix . $Email->table . $query;
                        // Use escapeshellarg to escape the shell argument
                        $escaped_command = 'mysql -h ' . escapeshellarg(DB_HOST) . ' -u ' . escapeshellarg(DB_USER) . ' -p' . escapeshellarg(DB_PASSWORD) . ' ' . escapeshellarg(DB_NAME) . ' -N -B -e ' . escapeshellarg($query) . ' | sed "s/\t/,/g" >> ' . escapeshellarg($outfile_full);
                        $exec = exec($escaped_command, $output);
            ...
            function admin_notices() {
                ...
                if (is_admin() && !defined('DOING_AJAX')) {
                    ...
                    $dismissed_smart_rating_forever = $this -> get_option( 'dismiss_forever-showmessage_ratereview', false ) ;


                    // Rate & Review
                    $showmessage_ratereview = $this -> get_option('showmessage_ratereview');
                    if ( !$dismissed_smart_rating_forever && !empty($showmessage_ratereview)) {
                        ...
                        $message = sprintf(__('You have been using %s for %s days or more. Please consider to %s on %s. We appreciate it very much! %s', 'wp-mailinglist'), '<a href="https://wordpress.org/plugins/newsletters-lite/" target="_blank">' . __('Tribulant Newsletters', 'wp-mailinglist') . '</a>', $showmessage_ratereview, '<a href="' . $rate_url . '" target="_blank" class="button"><i class="fa fa-star"></i> ' . __('leave your rating', 'wp-mailinglist') . '</a>', '<a href="https://wordpress.org/plugins/newsletters-lite/" target="_blank">WordPress.org</a>', '<button type="button" class="button notice-dismiss_forever">' . __('Dismiss forever', 'one-click-ssl') . '</button>');
                    ...
                    // Submit Serial
                    if   ((empty($hidemessage_submitserial)) && !is_array($serial_validation_status) && !$serial_validation_status && (empty($page) || $page != $this -> sections -> submitserial)) {
                    ...
                    switch ($method) {
                        ...
                        case 'activate'			:
                            ...
                            if (!empty($_GET[$this -> pre . 'subscriber_id']) && !empty($_GET['authkey'])) {
                                ...
                                if (!empty(  $subscriber)) {
                                    $Db -> model = $Subscriber -> model;
                                    $Db -> save_field('is_active', "Y", array('id' => $subscriber -> id));
                                }
                            ...
                            break;
                        case 'paypal'			:
                            global $Html, $SubscribersList, $Subscriber;
                                            ...
                                            switch ($txn_type) {
                                                ...
                                                case 'subscr_cancel'		:
                                                    ...
                                                    foreach ($mailinglists as $list_id) {
                                                        ...
                                                        $Subscriber -> save_field('is_active', "N", array('id' => $custom -> subscriber_id));
                            ...
                            if (!empty($_POST['order_number'])) {
                                if ($_POST['credit_card_processed'] == "Y") {
                                    ...
                                    if ($mykey === $_POST['key']) {
                                        ...
                                        $this -> autoresponders_send($subscriber, $mailinglist);
                                        $sl_conditions = array('subscriber_id' => $subscriber -> id, 'list_id' => $mailinglist -> id);
                                        $Subscriber -> save_field('is_active', "N", array('id' => $subscriber -> id));
                                        $SubscribersList -> save_field('active', "Y", $sl_conditions);
                                        $SubscribersList -> save_field('paid', "Y", $sl_conditions);
                    ...
                    switch ($activateaction) {
                        ...
                        case 'delete'				:
                            ...
                            if (!empty($activatedelete)) {
                                ...
                                $query = "SELECT * FROM `" . $wpdb->prefix . $Subscriber->table . "`    WHERE user_id = 0  AND `is_active` = 'N' AND `created` <= DATE_SUB(CURDATE(), INTERVAL " . $activatedelete . " DAY)  AND id NOT IN ( SELECT DISTINCT subscriber_id FROM `" . $wpdb->prefix . $SubscribersList->table . "` )";
                                if ($subscriptions = $wpdb -> get_results($query)) {
                                    foreach ($subscriptions as $subscription) {
                                        $Db -> model = $Subscriber -> model;
                                        $Db -> delete_all(array('id' => $subscription -> id));
                            ...
                            if (empty($minnumber) || (!empty($minnumber) && $allpostscount >= $minnumber)) {
                                if (!empty($posts) || !empty($shortcode_categories)) {
                                    ...
                                    if (!empty($preview) && $preview == true) {
                                        ...
                                        $message = $this -> render_email('send', array('message' => $content, 'subject' => $subject, 'subscriber' => $subscriber, 'history_id' => 0, 'post_id' => $post_id, 'eunique' => $eunique), false, $this -> htmltf($subscriber -> format), true, $latestpostssubscription -> theme_id);
                                        ...
                                        $output = ob_get_clean();
                                        ob_start();
                                        echo $this -> process_set_variables($subscriber, null, $output, 0);
                                        $output = ob_get_clean();
            ...
            function screen_settings($current, $screen) {
                ...
                if (!empty($page)) {
                    ...
                    if ($page == $this -> sections -> history) {
                        ...
                        // List of critical user meta keys that should never be modified directly
                        $forbidden_options = array(
                            'wp_capabilities',    // Prevents changing user roles
                            'user_level',         // Prevents changing user levels
                            'wp_user_level',      // WordPress specific user level
                            'admin_color',        // Prevents changing admin color scheme
                        );

                        //Whitelist of allowed options
                        $allowed_options = array(
                            'newsletters_history_perpage'
                        );


                        // Check if the option is in the allowed list
                        if (!in_array($option, $allowed_options, true)) {
                            return $current; // Abort if the option is not allowed
                        }

                        // Additional check to make sure only numeric values are allowed for pagination settings
                        if ($option === 'newsletters_history_perpage' && !is_numeric($value)) {
                            return $current; // Abort if the value is not a valid number
                        }

                        // Ensure the option starts with the 'newsletters_' prefix
                        if (strpos($option, 'newsletters_') !== 0) {
                            return $current; // Abort if the option does not start with the 'newsletters_' prefix
                        }

                        // Check if the option is forbidden
                        if (in_array($option, $forbidden_options, true)) {
                            return $current; // Abort if the option is in the forbidden list
                        }

                        // Ensure the user has permission to change this option
                        if (!current_user_can('edit_user', $user->ID)) {
                            return $current; // Abort if the user does not have sufficient permissions
                        }
                    ...
                    switch ($post_status) {
                        ...
                        case 'future'					:
                            if (!empty($_POST['newsletters_sendasnewsletter'])) {
                                ...
                                update_post_meta($post_id, '_newsletters_mailinglistsgroups', map_deep(wp_unslash($_POST['newsletters_mailinglistsgroups']), 'sanitize_text_field'));
                                update_post_meta($post_id, '_newsletters_mailinglistsroles', map_deep(wp_unslash($_POST['newsletters_mailinglistsroles']), 'sanitize_text_field'));
                                ...
                                delete_post_meta($post_id, '_newsletters_mailinglistsgroups');
                                delete_post_meta($post_id, '_newsletters_mailinglistsroles');
                        ...
                        case 'publish'					:
                            ...
                            if (!empty($_POST['newsletters_sendasnewsletter'])) {
                                $newsletters_mailinglists = isset($_POST['newsletters_mailinglists']) ? array_map('sanitize_text_field',wp_unslash($_POST['newsletters_mailinglists'])) : array();
                                $newsletters_mailinglistsgroups = isset($_POST['newsletters_mailinglistsgroups']) ? array_map('sanitize_text_field', wp_unslash($_POST['newsletters_mailinglistsgroups'])) : array();
                                $newsletters_mailinglistsroles = isset($_POST['newsletters_mailinglistsroles']) ? array_map('sanitize_text_field', wp_unslash($_POST['newsletters_mailinglistsroles'])) : array();
                                ...
                                if (!empty($newsletters_sendasnewsletter)) {
                                    ...
                                    $newsletters_mailinglistsgroups = get_post_meta($post_id, '_newsletters_mailinglistsgroups', true);
                                    $newsletters_mailinglistsroles = get_post_meta($post_id, '_newsletters_mailinglistsroles', true);
                            ...
                            if (!empty($newsletters_mailinglists) || !empty($newsletters_mailinglistsgroups) || !empty($newsletters_mailinglistsroles)) {                            // Setup the global post data
                                setup_postdata($post);
                                ...
                                $history_data = array(
                                    ...
                                    'roles'		        =>	maybe_serialize($newsletters_mailinglistsroles),
                                    'groups'		    =>	maybe_serialize($newsletters_mailinglistsgroups),
                                    'attachment'		=>	"N",
                                    'sent'				=>	1,
                                    'post_id'			=>	$post_id,
                                    'attachmentfile'	=>	false
                                ...
                                if(!empty($newsletters_mailinglistsgroups)) {
                                    $g = 1;
                                    $mlnlist = array();
                                    $querymalnlist = "SELECT DISTINCT " . $wpdb -> prefix . $Mailinglist -> table . ".id  FROM " . $wpdb -> prefix . $Mailinglist -> table . " where " . $wpdb -> prefix . $Mailinglist -> table . ".group_id in (" . implode("," , $newsletters_mailinglistsgroups) . " ); ";
                                    if (!empty($newsletters_mailinglists)) {
                                        $mailinglistscondition .= " or ";
                                    }
                                    if ($mlnlist = $wpdb -> get_results($querymalnlist)) {
                                        foreach ($mlnlist as $mailinglist_id) {
                                            $mailinglistscondition .= " " .  $wpdb->prefix . $SubscribersList->table . ".list_id = '" . $mailinglist_id->id . "'";
                                            if ($g < count($mlnlist)) {
                                                $mailinglistscondition .= " OR ";
                                            }
                                            $g++;
                                ...
                                if (isset($mailinglistscondition) && strlen($mailinglistscondition) > 1) {
                                    $query .=  $mailinglistscondition . ") AND " ;
                                }
                                $query .= $wpdb -> prefix . $SubscribersList -> table . ".active = 'Y'";
                                $subscribers = array();
                                if (isset($mailinglistscondition) && strlen($mailinglistscondition) > 1) {
                                    $subscribers = $wpdb->get_results($query);
                                }
                                if(!empty($newsletters_mailinglistsroles)) {
                                    // Define the user roles you want to filter by as an array
                                    $user_roles = $newsletters_mailinglistsroles; // Add your desired roles
                                    $users = array();
                                    ...
                                    if($filtered_users) {
                                        // Merge $subscribers with the results
                                        $mergedSubscribers = array_merge($subscribers, $filtered_users);
                                        // Create a new array with unique values based on the "email" field
                                        $uniqueSubscribers = array();
                                        $emailSet = array();
                                        foreach ($mergedSubscribers as $subscriber) {
                                            $email = $subscriber->email;
                                            if (!in_array($email, $emailSet)) {
                                                $uniqueSubscribers[] = $subscriber;
                                                $emailSet[] = $email;
                                            }
                                        }
                                        // Assign the unique results to $subscribers
                                ...
                                if ($subscribers) {
                                    ...
                                    foreach ($subscribers as $subscriber) {
                                        $this -> remove_server_limits();
                                        $queue_process_data = array();
                                        if (isset($subscriber->type) && !empty($subscriber->type) && $subscriber->type === "user") 
                                        {
                                            $queue_process_data = array(
            ...
            function admin_head_send() {
                ...
                $user_meta = get_user_meta( get_current_user_id(), 'metaboxhidden_newsletters_page_newsletters-create' ,true );
                // Check if the screen options have been saved. If so, use the saved value. Otherwise, use the default values.
                if ( $user_meta ) {
                    ?>
                    <script tye="javascript" >
                        document.addEventListener('DOMContentLoaded', function() {
                            // Array of metabox names to be hidden
                            var hiddenMetaboxes = <?php echo json_encode($user_meta); ?>;
                            // Hide the metaboxes based on their names
                            hiddenMetaboxes.forEach(function(metaboxName) {
                                var metaboxContainer = document.getElementById(metaboxName);
                                if (metaboxContainer) {
                                    metaboxContainer.style.display = 'none';
                                }
                            });
                        });
                    </script>
            ...
            function admin_forms() {
                ...
                switch ($method) {
                    case 'save'								:
                        ...
                        break;
                    case 'delete'							:
                        check_admin_referer($this -> sections -> forms . '_delete');
                    ...
                    default 								:
                        ...
                        if(!empty($data)) {
                            ...
                            if (WPMAIL()->language_do()) {
                                ...
                                foreach($data[$this->Subscribeform()->model] as $subscriberform_custom) {
                                    foreach ($subscriberform_custom as $ikey => $ival) {
                                        if (!is_array($ival) && !empty($ival)) {
                                            $subscriberform_custom->$ikey = WPMAIL()->language_use($language_live, $ival);
                                        }
                                    }
                                }
            ...
            function admin_autoresponders() {
                ...
                switch ($method) {
                    case 'save'					:
                        ...
                        break;
                    case 'delete'				:
                        check_admin_referer($this -> sections -> autoresponders . '_delete');
            ...
            function admin_mailinglists() {
                ...
                switch ($method) {
                    ...
                    case 'view'			:
                        ...
                        break;
                    case 'delete'		:
                        check_admin_referer($this -> sections -> lists . '_delete');
                        $id = (int) sanitize_text_field($_GET['id']);
                        if (!empty($id)) {
            ...
            function admin_groups() {
                ...
                switch ($method) {
                    ...
                    case 'view'						:
                        ...
                        break;
                    case 'delete'					:
                        check_admin_referer($this -> sections -> groups . '_delete');
            ...
            function admin_subscribers() {
                ...
                switch ($method) {
                    ...
                    case 'view'			:
                        ...
                        break;
                    case 'unsubscribe'	:
                        check_admin_referer($this -> sections -> subscribers . '_unsubscribe');
                        ...
                        break;
                    case 'delete'		:
                        check_admin_referer($this -> sections -> subscribers . '_delete');
                        $id = (int) sanitize_text_field(wp_unslash($_GET['id']));
                        if (!empty($id)) {
                            $Db -> model = $Subscriber -> model;
            ...
            function admin_history() {
                ...
                switch ($method) {
                        ...
                        if (!empty($id)) {
                            if ($history = $this -> History() -> get($id)) {
                                ...
                                if (!empty($this->referer)) {
                                    // Check if the URL contains the method=delete parameter
                                    // if (strpos($this->referer, 'method=delete') !== false) {
                                    $message = '';
                                    //}
                                }
                                $this -> redirect($this -> url, 'error', $message);
                    ...
                    case 'archive'			:
                        ...
                        break;
                    case 'delete'			:
                        check_admin_referer($this -> sections -> history . '_delete');
            ...
            function admin_links() {
                ...
                switch ($method) {
                    case 'delete'					:
                        check_admin_referer($this -> sections -> links . '_delete');
            ...
            function admin_clicks() {
                ...
                switch ($method) {
                    case 'delete'					:
                        check_admin_referer($this -> sections -> clicks . '_delete');
            ...
            function admin_orders() {
                ...
                switch ($method) {
                    ...
                    case 'save'			:
                        ...
                        break;
                    case 'delete'		:
                        check_admin_referer($this -> sections -> orders . '_delete');
            ...
            function admin_fields() {
                ...
                switch ($method) {
                    case 'save'				:
                        ...
                        break;
                    case 'delete'			:
                        check_admin_referer($this -> sections -> fields . '_delete');
                        $id = (int) sanitize_text_field($_GET['id']);
                        if (!empty($id)) {
                                        ...
                                        case 'tracking_image_file'			:
                                            ...
                                            if (!empty($_POST['tracking']) && $_POST['tracking'] == "Y" && !empty($_POST['tracking_image']) && $_POST['tracking_image'] == "custom") {
                                                if (!empty($_FILES['tracking_image_file']['name'])) {
                                                    ...
                                                    $_FILES['tracking_image_file'] = map_deep($_FILES['tracking_image_file'], 'sanitize_text_field');

                                                    $tracking_image_file = sanitize_text_field($_FILES['tracking_image_file']['name']);
                                                    ...
                                                    $file_extension = strtolower(pathinfo($tracking_image_file, PATHINFO_EXTENSION));

                                                    if (!in_array($file_extension, $allowed_extensions)) {
                                                        $this->render_error(__('Only PNG, JPG, and GIF files are allowed.', 'wp-mailinglist'));
                                                        exit();
                                                    }

                                                    // Allow only specific MIME types
                                                    $allowed_mime_types = array('image/png', 'image/jpeg', 'image/gif');
                                                    $file_mime_type = mime_content_type($_FILES['tracking_image_file']['tmp_name']);

                                                    if (!in_array($file_mime_type, $allowed_mime_types)) {
                                                        $this->render_error(__('Invalid file type. Only PNG, JPG, and GIF are allowed.', 'wp-mailinglist'));
                                                        exit();
                                                    }
                ...
                switch ($method) {
                    case 'runschedule'		:
                        if (!empty($_GET['hook'])) {
                            ...
                            switch ($hook) {
                                case 'WPML_WP_Queue_Process_cron'					:
                                    $this -> qp_do_crons();
                                    break;
    ...
    //This is needed on the webhosts that disable the referrer policy by default. 
    function additional_headers_on_admin_newsletters( $headers ) {   
        if (  is_admin() ) {
            $headers['Referrer-Policy']             = 'same-origin';
            return $headers; 
        }
    }
    add_filter( 'wp_headers', 'additional_headers_on_admin_newsletters' );
