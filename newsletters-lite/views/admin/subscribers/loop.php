		...
		<table class="widefat">
			<thead>
				<tr>
                    ...
                    echo wp_kses($cols_output , $allowed_html, $allowedProtocols);
			...
			<tfoot>
				<tr>
					<?php echo wp_kses($cols_output, $allowed_html, $allowedProtocols); ?>
				</tr>
			</tfoot>
			<tbody id="the-list">
									...
									case 'email'							:
										...
										<td>
											...
											<div class="row-actions">
												...
												<span class="delete"><a href="<?php echo esc_url_raw( wp_nonce_url(admin_url('admin.php?page=' . $this -> sections -> subscribers . '&method=delete&id=' . $subscriber -> id),  $this -> sections -> subscribers . '_delete')) ?>" onclick="if (!confirm('<?php esc_html_e('Are you sure you want to delete this subscriber?', 'wp-mailinglist'); ?>')) { return false; }" class="submitdelete"><?php esc_html_e('Delete', 'wp-mailinglist'); ?></a> |</span>
												<span class="edit"><span class="newsletters_warning"><a onclick="if (!confirm('<?php esc_html_e('Are you sure you want to unsubscribe this subscriber?', 'wp-mailinglist'); ?>')) { return false; }" href="<?php echo esc_url_raw( wp_nonce_url(admin_url('admin.php?page=' . $this -> sections -> subscribers . '&method=unsubscribe&id=' . $subscriber -> id),  $this -> sections -> subscribers . '_unsubscribe')) ?>"><?php esc_html_e('Unsubscribe', 'wp-mailinglist'); ?></a></span> |</span>
			...
			<div class="alignleft">
				<?php if (empty($_GET['showall'])) : ?>
					<select class="widefat" style="width:auto;" name="perpage" onchange="change_perpage(this.value);">
						...
						<?php $s = 5; ?>
						<?php while ($s <= 750) : ?>
							<option <?php echo (isset($_COOKIE[$this -> pre . 'subscribersperpage']) && $_COOKIE[$this -> pre . 'subscribersperpage'] == $s) ? 'selected="selected"' : ''; ?> value="<?php echo esc_html($s); ?>"><?php echo esc_html($s); ?> <?php _e('subscribers', 'wp-mailinglist'); ?></option>
							<?php if($s < 50) { $s += 5; } else { $s += 10; } ?>
						<?php endwhile; ?>
