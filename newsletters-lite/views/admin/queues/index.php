...
<div class="wrap newsletters <?php echo esc_html($this -> pre); ?> <?php echo esc_html( $this -> sections -> queue); ?>">
	...
	<div class="actions">
		...
		<?php if (!empty($count)) : ?>
			...
			<a href="<?php echo esc_url_raw( admin_url('admin.php?page=' . $this -> sections -> settings_tasks . '&method=runschedule&hook=WPML_WP_Queue_Process_cron')) ?>" onclick="if (!confirm('<?php esc_html_e('Are you sure you want to run the queue schedule now and send out emails?', 'wp-mailinglist'); ?>')) { return false; }" class="button"><i class="fa fa-send"></i> <?php esc_html_e('Run Now', 'wp-mailinglist'); ?></a>
		<?php endif; ?>
	</div>
	
	<?php if (!empty($count)) : ?>
		<p><?php echo wp_kses_post( $Html -> next_scheduled('WPML_WP_Queue_Process_cron')); ?></p>
	<?php endif; ?>
